...
class FileSystemStorage(DjangoFileSystemStorage):
    ...
    def url(self, name: str) -> str:
        ...
        if url.startswith("http"):
            return url
        if "/media/" in url:
            return settings.BASE_URL + url.lstrip("/")
        return settings.BASE_URL + self.base_url + url.lstrip("/")


file_system_storage: FileSystemStorage = FileSystemStorage()  # type: ignore
...
def base64_to_file(base64_data: str, file_name: str):
    ...
    file_name = "uploaded_file" if not file_name else file_name
    if "." not in file_name:
