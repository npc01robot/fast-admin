...
class Department(models.Model):
    ...
    def delete(self, using=None, keep_parents=False):
        with transaction.atomic(using=using):
            department_list = Department.objects.filter(parent=self)
            update_list = []
            for department in department_list:
                department.parent = self.parent
                update_list.append(department)
            Department.objects.bulk_update(update_list, ["parent"])
            self.is_deleted = True
            self.save()

    @classmethod
    def get_top_departments(cls, department: "Department"):
        """
        获取顶级部门
        """
        if department.parent is None:
            ...
            return cls.get_top_departments(department.parent)
