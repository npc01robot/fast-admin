...
onMounted(() => {
  if (id) {
    loading.value = true;
    getLoanInterestDetail(String(id)).then(res => {
...
const onSubmit = () => {
  formData.value.interest_amount = accounting.unformat(
    formData.value.interest_amount
  );
  if (id) {
    updateLoanInterest({ data: formData.value }, String(id)).then(res => {
      if (res && res.success) {
        isEdited.value = false;
        originalFormData.value = { ...formData.value };
        message("付息信息修改成功", { type: "success" });
        router.push({ name: "RepayInterest" });
      } else {
        message("付息信息修改失败！" + res.msg, { type: "error" });
      }
    });
  } else {
    createLoanInterest({ data: formData.value }).then(res => {
      console.log(useUserStoreHook().username);
      if (res && res.success) {
        isEdited.value = false;
        originalFormData.value = { ...formData.value };
        message("付息信息新增成功", { type: "success" });
        router.push({ name: "RepayInterest" });
      } else {
        message("付息信息新增失败!" + res.msg, { type: "error" });
      }
    });
  }
};
...
<template>
  <el-card shadow="never">
    ...
    <el-form
      ...
      :inline="true"
    >
      <el-form-item label="付息日期:">
        <el-date-picker
          v-model="formData.interest_date"
          type="date"
          placeholder="选择日期"
      ...
      <el-form-item label="付息来源:">
        <el-input
          v-model="formData.interest_source"
          placeholder="请输入付息来源"
        />
      </el-form-item>
